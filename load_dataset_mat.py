...
class SkeletonDatasetMAT(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        data = {
            'skeleton': torch.tensor(mat_data['skeleton']).squeeze(0),
            'image': torch.tensor(mat_data['image']),
            'gender': torch.tensor(mat_data['gender']).squeeze(0),
            'trans' : torch.tensor(mat_data['trans']).squeeze(0), # 根节点偏移量
        }
        # 做维度的调整等
        if self.transform:
    ...
    def _transform_data(self, data):
        ...
        # data['gender'] = torch.tensor(data['gender'], dtype = torch.uint8)

        return data


if __name__ == "__main__":
    dataset = SkeletonDatasetMAT(r'F:\pose_master_dataset_mat\train')
    import cv2
    import numpy as np
