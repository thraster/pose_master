...
def train(train_loader, test_loader,model = SMLPR_MLP, num_epochs=10,  checkpoint_path = None):
    ...
    model_m = model(device)
    model_f = model(device)
    ...
    optimizer1 = optim.Adam(model_m.parameters(), lr=0.001, weight_decay=0.0001)
    optimizer2 = optim.Adam(model_f.parameters(), lr=0.001, weight_decay=0.0001)
    ...
    writer = SummaryWriter(log_dir=f'log/{model_m.name}/exp{current_date}')
    ...
    for epoch in range(total_epoch, num_epochs):
        ...
        for i, data in enumerate(train_loader):
            ...
            # break
            shape_loss_f.backward()
            pose_loss_f.backward()
            optimizer1.step()

            shape_loss_m.backward()
            pose_loss_m.backward()
            optimizer2.step()
            # break
            # 打印统计信息

            running_loss_shape += shape_loss_f.item()+shape_loss_m.item()
            running_loss_pose += pose_loss_f.item()+pose_loss_m.item()
        ...
        test_loss_shape, test_loss_pose = test_model(model_f=model_f,model_m=model_m,test_loader=test_loader,device=device,criterion=criterion)
