...
def train(train_loader, test_loader, num_epochs=10, model = mobilenet_refine_beta, checkpoint_path = None):
    '''
    ...
    writer = {
        ...
        'MPJPE': SummaryWriter(f"/root/tf-logs/{model.name}/exp{current_date}"),
    ...
    for epoch in range(total_epoch, num_epochs):
        ...
        for i, data in enumerate(train_loader):
            ...
            writer['loss(train)'].add_scalar(tag=f"loss(training) L1Loss", scalar_value=loss,
        ...
        # 添加标量 loss when test
        writer['MPJPE'].add_scalar(tag="MPJPE(testing) /mm", scalar_value=test_loss,
                global_step=epoch)
        ...
        torch.save(checkpoint, f'/root/pose_master/my_checkpoints/beta/last_{model.name}.pth')
        print(f"last checkpoint saved! test loss = {test_loss}")
        
        # 每50个epoch储存一下？
        if epoch%50 == 0:
            torch.save(checkpoint, f'/root/pose_master/my_checkpoints/beta/{model.name}_epoch{epoch}.pth')
        ...
        if epoch == 0:
            ...
            # flag = 0
            min_loss = test_loss
            torch.save(checkpoint, f'/root/pose_master/my_checkpoints/beta/best_{model.name}.pth')
...
def parse_args():
    ...
    # 添加命令行参数
    parser.add_argument('--train_data_path', type=str, default='/root/pose_master/dataset/train_lmdb_gt/', help='Path to the training LMDB dataset')
    parser.add_argument('--test_data_path', type=str, default='/root/pose_master/dataset/test_imdb_gt/', help='Path to the testing LMDB dataset')
...
if __name__ == "__main__":
    ...
    train(train_loader = train_loader , test_loader = test_loader, num_epochs=200, model=mobilenet_refine_beta,checkpoint_path = '/root/pose_master/my_checkpoints/beta/last_pretrained_mobilenetv2_beta_refine.pth')
