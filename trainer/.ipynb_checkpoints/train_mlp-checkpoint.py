...
def train(train_loader, test_loader,model = SMLPR_MLP, num_epochs=10,  checkpoint_path = None):
    ...
    if checkpoint_path != None:
        ...
        min_shape_loss = checkpoint['shape loss']
        min_pose_loss = checkpoint['pose loss']
        min_loss = min_shape_loss + min_pose_loss
    ...
    for epoch in range(total_epoch, num_epochs):
        ...
        torch.save(checkpoint, f'/root/pose_master/my_checkpoints/mlp/last_{model.name}_m.pth')
        ...
        # 每50个epoch储存一下？
        if epoch%50 == 0:
            torch.save(checkpoint, f'/root/pose_master/my_checkpoints/mlp/f{model.name}_epoch{epoch}_m.pth')
        ...
        if epoch == 0:
            ...
            torch.save(checkpoint, f'/root/pose_master/my_checkpoints/mlp/best_{model.name}_m.pth')
    ...
    writer.close()
    print("训练完成")
    for shape_l, pose_l in zip(test_loss_shape, test_loss_pose):
        print(f"shape loss = {shape_l}, pose loss = {pose_l}")
    total_time = 0.0
...
if __name__ == "__main__":
    ...
    train(train_loader = train_loader , test_loader = test_loader, num_epochs=101, model=SMLPR_MLP,checkpoint_path = '/root/pose_master/my_checkpoints/mlp/last_SMLPR_MLP_layer_m.pth')
